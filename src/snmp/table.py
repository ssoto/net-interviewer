...
class Snmp_table_request:
    ...
    def request(self):
        ...
        if self.oid_name == 'cDot11ClientConfigInfoTable' and self.mib_name == 'CISCO-DOT11-ASSOCIATION-MIB':
            ...
            raise NotImplementedError('develop all the snmptable command and parse stuff')

    def get_json_reply(self):
        if not hasattr(self, 'raw_output'):
            raise AttributeError('request output has not been established... Request has not been done or failed')
        output_list = [ x.split(',') for x in self.raw_output.split('\n') ]
        # self.num_rows attribute is an optimization stuff: len(header) is been only calculated onece
        self.num_rows = len(output_list[0])
        header = output_list[0]
        dict_result = self._get_body_dict(header, output_list[1:])
        return dict_result
    ...
    def _get_body_dict(self, header, elements):
        ...
        for elements_index in range(0, num_elements):
            ...
            for index in range(0, self.num_rows): 
                element_dict[header[index]] = (elements[elements_index][index]).translate(None, '"').strip()
            result[elements_index] = element_dict
        return result

class MyAppLookupError(RuntimeWarning):
